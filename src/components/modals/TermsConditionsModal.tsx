/**
 * Companies → Terms & Conditions — load/save via `/setting/terms-conditions`.
 */
...
import {
  ...
  saveTermsConditions,
  type TermsConditionsSettings,
  type TermsDocKey,
} from "@/services/termsConditionsApi";
...
export const TermsConditionsModal: React.FC<TermsConditionsModalProps> = ({ onClose }) => {
  const [values, setValues] = useState<TermsConditionsSettings>({ ...EMPTY_TERMS });
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setValues(await fetchTermsConditions());
    } catch (err) {
      showToast(errMsg(err, "Couldn't load terms & conditions"), "error");
      setValues({ ...EMPTY_TERMS });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const patch = (key: TermsDocKey, value: string) =>
  ...
  const onDone = async () => {
    ...
    setSaving(true);
    try {
      const saved = await saveTermsConditions(values);
      setValues(saved);
      showToast("Terms & conditions saved", "success");
      onClose();
    } catch (err) {
      showToast(errMsg(err, "Couldn't save terms & conditions"), "error");
    } finally {
      setSaving(false);
    }
  };
