/**
 * Credit Notes — /api/v1/account/credit-notes
 */
...
export const AccountsCreditNotes: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["credit-notes", page, perPage, search, sortAsc, statusFilter],
    queryFn: () =>
      fetchCreditNotes({
        page,
        limit: perPage,
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ["credit-notes"] });

  const openView = async (r: NoteRow) => {
    try {
      setView(await fetchCreditNote(r.id));
    } catch (e: any) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteCreditNote(deleteTarget.id);
  ...
  return (
    <>
        ...
        <table className="w-full text-sm min-w-[850px]">
          <thead className="bg-white sticky top-0 z-10 border-b border-gray-200">
            <tr>
              ...
              {["Number", "Customer", "Amount", "Status", "Actions"].map((h) => (
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            ...
            {!isLoading && rows.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-500">No credit notes found.</td></tr>
            )}
            {isLoading && (
      ...
      {view && (
        <ModalShell title="Credit Note Details" onClose={() => setView(null)} onSubmit={() => setView(null)} submitLabel="Close">
          <div className="space-y-2 text-sm text-gray-700">
            <p><span className="text-gray-500">Number:</span> {view.credit_note_number || view.note_number || "—"}</p>
      ...
      {deleteTarget && (
        <DeleteConfirm what="credit note" name={deleteTarget.number || deleteTarget.partyName} onConfirm={() => void confirmDelete()} onCancel={() => setDeleteTarget(null)} />
      )}
    </>
  );
};
