/**
 * Accounting Customers — /customer/all (server pagination)
 */
...
type Row = { id: string; name: string; email: string; phone: string; companyName: string };

const mapRow = (c: any): Row => ({
  id: String(c._id),
  name: c.name || "",
  email: c.email || "",
  phone: c.phone || "",
  companyName: c.businessProfile?.companyName || c.company_name || "",
});
...
export const AccountsCustomers: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["account-customers", page, perPage, search, sortAsc],
    queryFn: async () => {
      const res = await api.raw.get("/customer/all", {
        params: {
          page,
          limit: perPage,
      ...
      const body = res.data ?? {};
      const rows = Array.isArray(body.data) ? body.data.map(mapRow) : [];
      return { rows, pagination: body.pagination ?? { totalData: rows.length } };
    },
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ["account-customers"] });
  ...
  const submit = async () => {
    ...
    try {
      if (modal === "edit" && editingId) await api.post("/customer/update", { _id: editingId, ...payload });
      else await api.post("/customer/create", payload);
      showToast(modal === "edit" ? "Customer updated" : "Customer created", "success");
      setModal(null);
      await invalidate();
    } catch (e: any) {
  ...
  return (
    <>
              ...
              <tr key={r.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3.5">
                  <div className="flex items-center gap-1.5">
                    <button type="button" onClick={() => { setForm({ ...emptyForm(), name: r.name, email: r.email, phone: r.phone, company_name: r.companyName }); setModal("view"); }} className="p-1.5 text-gray-400 hover:text-indigo-600 rounded hover:bg-indigo-50"><Eye className="w-4 h-4" /></button>
                    <button type="button" onClick={() => { setEditingId(r.id); setForm({ ...emptyForm(), name: r.name, email: r.email, phone: r.phone, company_name: r.companyName }); setModal("edit"); }} className="p-1.5 text-gray-400 hover:text-blue-600 rounded hover:bg-blue-50"><Edit className="w-4 h-4" /></button>
                    <button type="button" onClick={() => setDeleteTarget(r)} className="p-1.5 text-gray-400 hover:text-red-600 rounded hover:bg-red-50"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </td>
              </tr>
            ))}
            {!isLoading && rows.length === 0 && <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-500">No customers found.</td></tr>}
      ...
      {(modal === "create" || modal === "edit") && (
        <ModalShell title={modal === "edit" ? "Edit Customer" : "Create Customer"} onClose={() => setModal(null)} onSubmit={submit} submitLabel={modal === "edit" ? "Update" : "Create"}>
      ...
      {modal === "view" && (
        <ModalShell title="Customer Details" onClose={() => setModal(null)} onSubmit={() => setModal(null)} submitLabel="Close">
      ...
      {deleteTarget && (
        <DeleteConfirm what="customer" name={deleteTarget.name} onConfirm={() => void confirmDelete()} onCancel={() => setDeleteTarget(null)} />
      )}
    </>
  );
};
