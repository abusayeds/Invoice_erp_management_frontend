/**
 * File: src/pages/crm/LeadDetail.tsx
 * CRM Lead detail — reached from the eye icon on the Leads list (/crm/leads/:id).
 * Vertical tab rail (General · Tasks · Users · Products · Sources · Files ·
 * Calls · Activity) rebuilt from the ERPGO reference screenshots, but in the
 * Qayd light/blue theme (brand blue #007aff instead of the reference green).
 * All sub-data (tasks/users/products/sources/calls/emails/discussions/notes)
 * persists per-lead in the Dexie `meta` table via src/lib/db/leadDetail.ts.
 */
...
export const LeadDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    api.raw.get(`/crm/leads/${id}`).then((res) => {
      const l = (res.data?.data ?? res.data) as any;
      if (!l || !l._id) return;
  ...
  const seed = useMemo(
    () => leadDetailFromApi({ assigned_users: (lead?.assignedTo ?? []).map((n) => ({ name: n })) }),
    [lead],
  );

  const stored = useLeadDetail(id);
  const data: LeadDetailData = stored ?? seed;

  // Seed the Dexie row the first time this lead is opened.
  useEffect(() => {
    if (id && stored === null) {
      saveLeadDetail(id, seed);
    }
  ...
  const commit = (next: LeadDetailData) => {
    if (id) {
      saveLeadDetail(id, next); // optimistic local update
      void syncLeadDetail(id, data, next); // push to backend, then mirror server truth
    }
  };
  const logActivity = (
    base: LeadDetailData,
  ...
  const [pickerModal, setPickerModal] = useState<null | "users" | "products" | "sources">(null);
  ...
  const saveTask = () => {
    ...
    if (editingTask) {
      commit({
        ...data,
        tasks: data.tasks.map((t) => (t.id === editingTask.id ? { ...t, ...taskForm } : t)),
      });
      showToast("Task updated", "success");
    } else {
      commit({
        ...data,
        tasks: [{ id: uid(), ...taskForm }, ...data.tasks],
        activity: logActivity(data, "task", taskForm.name),
      });
  ...
  const deleteTask = (tid: string) =>
    commit({ ...data, tasks: data.tasks.filter((t) => t.id !== tid) });
  ...
  const saveCall = () => {
    ...
    if (editingCall) {
      commit({
        ...data,
        calls: data.calls.map((c) => (c.id === editingCall.id ? { ...c, ...callForm } : c)),
      });
      showToast("Call updated", "success");
    } else {
      commit({
        ...data,
        calls: [{ id: uid(), ...callForm }, ...data.calls],
        activity: logActivity(data, "call", "Create new Lead Call"),
      });
      showToast("Call created", "success");
    }
    setCallModal(false);
  };
  const deleteCall = (cid: string) =>
    commit({ ...data, calls: data.calls.filter((c) => c.id !== cid) });
  ...
  const sendEmail = () => {
    ...
    commit({
      ...
      activity: logActivity(data, "email", "Create new Lead Email"),
    });
  ...
  const InfoRow: React.FC<{
    ...
    <div className="flex items-start gap-3">
      ...
      <div>
        <div className="text-[11px] tracking-wide text-gray-400 font-medium uppercase">
          {label}
        </div>
      ...
      case "General":
        return (
          <div className="space-y-5">
            ...
            {/* Lead info card */}
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <div className="bg-blue-50/70 px-5 py-4 border-b border-blue-100">
                <h2 className="text-lg font-semibold text-gray-900">{lead.name}</h2>
                <p className="text-sm text-gray-500 mt-0.5">{lead.subjects}</p>
              </div>
              <div className="p-5 grid grid-cols-1 md:grid-cols-3 gap-5">
                <InfoRow icon={Mail} label="Email" value={lead.email} />
                <InfoRow icon={Phone} label="Phone" value={lead.phone} />
                <InfoRow icon={Calendar} label="Follow Up Date" value={lead.followUpDate} />
                <InfoRow icon={GitBranch} label="Pipeline" value={pipeline} />
                <InfoRow icon={Layers} label="Stage" value={lead.stage} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              <SectionCard title="Emails" onAdd={() => setEmailModal(true)}>
                {data.emails.length === 0 ? (
                  <div className="px-5 pb-8 pt-2 text-center text-sm text-gray-400">
                    No emails found
                  </div>
                ) : (
                      ...
                      <div key={e.id} className="px-5 py-3">
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-900 flex items-center gap-2">
                            <Mail className="w-4 h-4 text-gray-400" /> {e.to}
                          </span>
                    ...
                    <tr key={t.id}>
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          <button onClick={() => openEditTask(t)} className="text-blue-600 hover:text-blue-800">
                ...
                <tbody className="divide-y divide-gray-100">
                  {data.users.map((u) => (
                    <tr key={u.id}>
                      ...
                      <td className="px-5 py-3">
                        <button onClick={() => removeNamed("users", u.id)} className="text-red-500 hover:text-red-700">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </td>
                    </tr>
                ...
                <tbody className="divide-y divide-gray-100">
                  {data.products.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td className="px-5 py-3">
                        <button onClick={() => removeNamed("products", p.id)} className="text-red-500 hover:text-red-700">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </td>
                    </tr>
                ...
                <tbody className="divide-y divide-gray-100">
                  {data.sources.map((s) => (
                    <tr key={s.id}>
                      ...
                      <td className="px-5 py-3">
                        <button onClick={() => removeNamed("sources", s.id)} className="text-red-500 hover:text-red-700">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </td>
                    </tr>
                ...
                <tbody className="divide-y divide-gray-100">
                  {data.calls.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          <button onClick={() => openEditCall(c)} className="text-blue-600 hover:text-blue-800">
                            <Pencil className="w-4 h-4" />
                          </button>
                          <button onClick={() => deleteCall(c.id)} className="text-red-500 hover:text-red-700">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                      </td>
    ...
    <div className="flex-1 bg-[#FAFBFC] overflow-hidden flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 px-4 sm:px-6 py-2">
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span>›</span>
          <button onClick={() => navigate("/crm/leads")} className="hover:text-gray-700">Lead</button>
          <span>›</span>
        ...
        <div className="fixed inset-0 z-[80] flex items-start justify-center p-4 pt-24 bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <button onClick={() => setTaskModal(false)} className="p-1.5 hover:bg-gray-100 rounded-lg">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setTaskForm({ ...taskForm, priority: e.target.value as LeadTask["priority"] })}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setTaskForm({ ...taskForm, status: e.target.value as LeadTask["status"] })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm bg-white"
                  >
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
              <button onClick={() => setTaskModal(false)} className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
                Cancel
              </button>
              <button onClick={saveTask} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
        ...
        <div className="fixed inset-0 z-[80] flex items-start justify-center p-4 pt-16 bg-black/40 overflow-y-auto">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <button onClick={() => setCallModal(false)} className="p-1.5 hover:bg-gray-100 rounded-lg">
                <X className="w-5 h-5 text-gray-500" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setCallForm({ ...callForm, callType: e.target.value as LeadCall["callType"] })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm bg-white"
                  >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setCallForm({ ...callForm, assignee: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm bg-white"
                >
              ...
              <div>
                ...
                <RichEditor
                  ...
                  onChange={(html) => setCallForm({ ...callForm, result: html })}
                  minHeight={120}
                />
              </div>
            </div>
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
              <button onClick={() => setCallModal(false)} className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
                Cancel
              </button>
              <button onClick={saveCall} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
        ...
        <div className="fixed inset-0 z-[80] flex items-start justify-center p-4 pt-16 bg-black/40 overflow-y-auto">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <button onClick={() => setEmailModal(false)} className="p-1.5 hover:bg-gray-100 rounded-lg">
                <X className="w-5 h-5 text-gray-500" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <RichEditor
                  ...
                  onChange={(html) => setEmailForm({ ...emailForm, description: html })}
                  minHeight={140}
                />
              </div>
            </div>
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
              <button onClick={() => setEmailModal(false)} className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
                Cancel
              </button>
              <button onClick={sendEmail} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                Send Email
              </button>
            </div>
          </div>
      ...
      {/* ── Add Message (discussion) modal ── */}
      {discussionModal && (
        <div className="fixed inset-0 z-[80] flex items-start justify-center p-4 pt-24 bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">Add Message</h3>
              <button onClick={() => setDiscussionModal(false)} className="p-1.5 hover:bg-gray-100 rounded-lg">
                <X className="w-5 h-5 text-gray-500" />
              </button>
            </div>
            <div className="p-6">
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
              <button onClick={() => setDiscussionModal(false)} className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
                Cancel
              </button>
              <button onClick={addDiscussion} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                Save
              </button>
            </div>
          </div>
