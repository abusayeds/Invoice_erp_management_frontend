...
export const JobLocations: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      const res = await api.raw.get("/recruitment/job-locations/all");
      const arr = toArray<any>(res.data);
      setLocations(arr.map((l: any) => ({
        id: String(l._id),
        name: l.name ?? "",
        address: l.address ?? "",
        city: l.city ?? "",
        ...
        type: l.remote_work ? "Remote" : "On-site",
        status: l.status === "Inactive" ? "Inactive" : "Active",
        createdAt: l.createdAt ? String(l.createdAt).slice(0, 10) : "",
      })) as JobLocation[]);
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      remote_work: formData.type === "Remote",
      status: formData.status,
    };
    try {
      if (isEditing && selectedLocation) {
        await api.raw.patch(`/recruitment/job-locations/edit/${selectedLocation.id}`, payload);
        showToast("Job location updated successfully!", "success");
        setShowEditModal(false);
      } else {
        await api.raw.post("/recruitment/job-locations/create", payload);
      ...
      showToast("Failed to save job location", "error");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.raw.delete(`/recruitment/job-locations/delete/${selectedLocation.id}`);
      showToast("Job location deleted successfully!", "success");
      setShowDeleteModal(false);
      setSelectedLocation(null);
      await load();
    } catch {
      showToast("Failed to delete job location", "error");
    }
  };
