...
export const InterviewRounds: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      const res = await api.raw.get("/recruitment/interview-rounds/all");
      const arr = toArray<any>(res.data);
      const rid = (v: any) => (v && typeof v === "object" ? String(v._id) : String(v ?? ""));
      setRounds(arr.map((r: any) => ({
        id: String(r._id),
        name: r.name ?? "",
        jobId: rid(r.job_id),
        jobTitle: (r.job_id && typeof r.job_id === "object" ? (r.job_id.title ?? r.job_id.job_title) : "") || r.job_posting || "",
        sequence: Number(r.sequence_number) || 0,
        description: r.description ?? "",
        status: r.status === "Inactive" ? "Inactive" : "Active",
        createdAt: r.createdAt ? String(r.createdAt).slice(0, 10) : "",
      })) as InterviewRound[]);
    } catch { /* leave list as-is on error (no mock) */ }
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      sequence_number: formData.sequence,
      description: formData.description.trim(),
      status: formData.status,
    };
    if (!selectedRound) return;
    try {
      await api.raw.patch(`/recruitment/interview-rounds/edit/${selectedRound.id}`, payload);
      showToast("Interview round updated successfully!", "success");
      setShowEditModal(false);
      await load();
      resetForm();
    } catch {
      showToast("Failed to save interview round", "error");
    }
  };
  ...
  const handleDelete = async () => {
    if (!selectedRound) return;
    try {
      await api.raw.delete(`/recruitment/interview-rounds/delete/${selectedRound.id}`);
      showToast("Interview round deleted successfully!", "success");
      setShowDeleteModal(false);
      setSelectedRound(null);
      await load();
    } catch {
      showToast("Failed to delete interview round", "error");
    }
  };
