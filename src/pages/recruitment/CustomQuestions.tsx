...
export const CustomQuestions: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      const res = await api.raw.get("/recruitment/custom-questions/all");
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && selectedQuestion) {
        await api.raw.patch(`/recruitment/custom-questions/edit/${selectedQuestion.id}`, payload);
        showToast("Custom question updated successfully!", "success");
        setShowEditModal(false);
      } else {
        await api.raw.post("/recruitment/custom-questions/create", payload);
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.raw.delete(`/recruitment/custom-questions/delete/${selectedQuestion.id}`);
      showToast("Custom question deleted successfully!", "success");
      setShowDeleteModal(false);
      setSelectedQuestion(null);
      await load();
    } catch {
      showToast("Failed to delete custom question", "error");
    }
  };
